...
Chromosome::Chromosome(int size, int start, int inpopulations) : Genome(size)
{
  ...
  StateArrivalProbs = new double**[size];
  for(int t = 1; t < size; t++ ){        
    StateArrivalProbs[t] = alloc2D_d(populations,2);
  }
  //create vector of jump indicators
  vector<bool> true_vector(size,true);
  xi.assign(2,true_vector);
...
void Chromosome::SampleNumberOfArrivals(const unsigned int gametes, const bool isX, 
					unsigned int SumN[], unsigned int SumN_X[]){
  int locus = _startLocus+1;
