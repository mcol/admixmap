...
Main.Func <- function()
{
   ...
   if (nullflag == 1)
   {
      ...
      lamhatmarg <- max(median(tnullmarg[,2]^2,na.rm=T)/0.456, 1)
      lamhatint <- NULL
      if (margint == 4)
      {     
         lamhatint  <- max(median(tnullint[,3]^2,na.rm=T)/0.456, 1)
         lamhatomn  <- max(median(tnullint[,4],  na.rm=T)/0.79, 1)
      }
   ...
   for (i in 1:length(tempmodel))
   {
      ...
      {      
         ...
         if (dim(clmtemp)[1] == (4+ncov))
         {
            ...
            pvals[i,2]         <- 1-pchisq((stats[i,2]^2),df=1)
            pvals[i,3]         <- 1-pchisq((stats[i,3]^2),df=1)
            pvals[i,4]         <- 1-pchisq((stats[i,4]^2),df=1)
            ...
            den     <- denss/dendf
            stats[i,5]  <- num / den         
            pvals[i,5]  <- 1-pf((stats[i,5]/lamhatomn),numdf,dendf)
         }
         else stats[i,1] <- -9
      }
   }
   ...
   cat("\n\n--------------------------------------------------------------------------", file=outmodel, append=T)
   cat("\nProgram: GC", file=outmodel, append=T)
   ...
   if ((nullflag == 1) & (is.na(outnull) == F))
   {
      ...
      cat("\n\n--------------------------------------------------------------------------", file=outnull, append=T)
      cat("\nProgram: GC", file=outnull, append=T)
