...
rho2 <- 2
spacing <- 2 # 40 cM spacing gives 99 loci
...
pbar <- 0.8
for(locus in 1:L) {
  alleleFreqs[2*locus - 1, 1] <- 0.99 # rbeta(1, 10*pbar, 10*(1-pbar))
  alleleFreqs[2*locus - 1, 2] <- 0.01 # rbeta(1, 10*(1-pbar), 10*pbar)
                                        # freqs allele 1 
...
write.table(genotypes, file="sim/data/genotypes.txt", sep="\t",
            row.names=FALSE)
...
write.table(loci, file="sim/data/loci.txt", sep="\t", row.names=FALSE)
...
write.table(trueallelefreqs, file="sim/data/trueallelefreqs.txt", sep="\t",
            row.names=FALSE)                               
