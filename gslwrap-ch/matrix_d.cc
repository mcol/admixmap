/*         DO NOT EDIT THIS FILE!!        */
/* -- AUTOGENERATED FROM *_source FILE -- */
#include "rand.h"

#include "vector.h"
#include "vector_d.h"
#include "vector_i.h"

#include "matrix.h"
#include "matrix_d.h"
#include "matrix_i.h"

#define MATRIX_TYPE_d
#define BOUNDS_CHECK 1

using namespace std;

Matrix_d::Matrix_d():
  _matrix((gsl_matrix*)malloc(sizeof(gsl_matrix)))
{
  assert(_matrix);
  _matrix->size1 = 0;
  _matrix->size2 = 0;
  _matrix->tda   = 0;
  _matrix->data  = 0;
  _matrix->block = 0;
  _matrix->owner = 0;
}

Matrix_d::Matrix_d(size_t NewRows, size_t NewCols):
  _matrix(gsl_matrix_calloc(NewRows,NewCols))
{
  assert(NewRows != 0);
  assert(NewCols != 0);
  assert(_matrix);
}

Matrix_d::Matrix_d(const Matrix_d& other):
  _matrix(gsl_matrix_calloc(other.GetNumberOfRows(),other.GetNumberOfCols()))
{
  _MissingRow = other._MissingRow;
  _MissingCol = other._MissingCol;
  gsl_matrix_memcpy(_matrix,other._matrix);
}

Matrix_d::~Matrix_d()
{
  gsl_matrix_free( _matrix );
  _matrix = 0;
}

Matrix_d &Matrix_d::operator=( const Matrix& m )
{
  if((void*)this == (void*)&m){
    return *this;
  }

  if (GetNumberOfRows() != m.GetNumberOfRows() ||
      GetNumberOfCols() != m.GetNumberOfCols()) {
    gsl_matrix_free( _matrix );
    _matrix = gsl_matrix_calloc(m.GetNumberOfRows(),m.GetNumberOfCols());
  }
  
  _MissingRow.clear();
  _MissingCol.clear();
  for (int i = 0; i < m.GetNumberOfMissingValues(); i++ ){
    _MissingRow.push_back(m.GetMissingValueRow(i));
    _MissingCol.push_back(m.GetMissingValueCol(i));
  }

  for(int i=0; i<GetNumberOfRows(); i++){
    for(int j=0; j<GetNumberOfCols(); j++){
      (*this)(i,j) = static_cast<double>(m(i,j));
    }
  }
  
  return *this;
}

Matrix_d &Matrix_d::operator=( const Matrix_d& m )
{
  if((void*)this == (void*)&m){
    return *this;
  }

  if (GetNumberOfRows() != m.GetNumberOfRows() ||
      GetNumberOfCols() != m.GetNumberOfCols()) {
    gsl_matrix_free( _matrix );
    _matrix = gsl_matrix_calloc(m.GetNumberOfRows(),m.GetNumberOfCols());
  }
  
  _MissingRow.clear();
  _MissingCol.clear();
  for (int i = 0; i < m.GetNumberOfMissingValues(); i++ ){
    _MissingRow.push_back(m.GetMissingValueRow(i));
    _MissingCol.push_back(m.GetMissingValueCol(i));
  }

  for(int i=0; i<GetNumberOfRows(); i++){
    for(int j=0; j<GetNumberOfCols(); j++){
      (*this)(i,j) = static_cast<double>(m(i,j));
    }
  }
  
  return *this;
}

Matrix_d &Matrix_d::operator=( const Matrix_i& m )
{
  if((void*)this == (void*)&m){
    return *this;
  }

  if (GetNumberOfRows() != m.GetNumberOfRows() ||
      GetNumberOfCols() != m.GetNumberOfCols()) {
    gsl_matrix_free( _matrix );
    _matrix = gsl_matrix_calloc(m.GetNumberOfRows(),m.GetNumberOfCols());
  }
  
  _MissingRow.clear();
  _MissingCol.clear();
  for (int i = 0; i < m.GetNumberOfMissingValues(); i++ ){
    _MissingRow.push_back(m.GetMissingValueRow(i));
    _MissingCol.push_back(m.GetMissingValueCol(i));
  }

  for(int i=0; i<GetNumberOfRows(); i++){
    for(int j=0; j<GetNumberOfCols(); j++){
      (*this)(i,j) = static_cast<double>(m(i,j));
    }
  }
  
  return *this;
}

double&
Matrix_d::operator()( int row, int col )
{
  return _matrix->data[row * GetNumberOfCols() + col];
}

double&
Matrix_d::operator()( int row, int col ) const
{
  return _matrix->data[row * GetNumberOfCols() + col];
}

Matrix_d Matrix_d::Double() const
{
  Matrix_d m(GetNumberOfRows(),GetNumberOfCols());

  for(int i=0; i<GetNumberOfRows(); i++){
    for(int j=0; j<GetNumberOfCols(); j++){
      m(i,j) = (double)(*this)(i,j);
    }
  }
  return(m);
}

Matrix Matrix_d::Float() const
{
  Matrix m(GetNumberOfRows(),GetNumberOfCols());

  for(int i=0; i<GetNumberOfRows(); i++){
    for(int j=0; j<GetNumberOfCols(); j++){
      m(i,j) = (float)(*this)(i,j);
    }
  }
  return(m);
}

Matrix_i Matrix_d::Integer() const
{
  Matrix_i m(GetNumberOfRows(),GetNumberOfCols());

  for(int i=0; i<GetNumberOfRows(); i++){
    for(int j=0; j<GetNumberOfCols(); j++){
      m(i,j) = (int)(*this)(i,j);
    }
  }
  return(m);
}

int Matrix_d::GetNumberOfRows() const
{
  return _matrix->size1;
}

int Matrix_d::GetNumberOfCols() const
{
  return _matrix->size2;
}

void Matrix_d::SetNumberOfElements(size_t NewRows, size_t NewCols)
{
  _MissingRow.clear();
  _MissingCol.clear();

  assert( NewRows != 0 );
  assert( NewCols != 0 );
  if(NewRows==0 || NewCols==0){
    cerr << endl << endl << endl;
    cerr << "BUG - Matrix_d::SetNumberOfElements(size_t,size_t) was passed zero as an argument... but continuing" << endl;
    cerr << "BUG -   first arg==" << NewRows << "; second arg==" << NewCols << endl;
    if(NewRows==0){
      NewRows=1;
      cerr << "FIXME: setting first arg to 1" << endl;
    }
    if(NewCols==0){
      NewCols=1;
      cerr << "FIXME: setting second arg to 1" << endl;
    }
    cerr << endl << endl << endl;
  }


  if (GetNumberOfRows() != (int)NewRows ||
      GetNumberOfCols() != (int)NewCols ){
    gsl_matrix_free( _matrix );
    _matrix = gsl_matrix_calloc( NewRows, NewCols );
  } else {
    SetElements(static_cast<double>(0.0));
  }
}

void Matrix_d::SetElements(double NewValue)
{
  for (int i=0; i<GetNumberOfRows(); i++){
    for (int j=0; j<GetNumberOfCols(); j++){
      (*this)(i,j) = NewValue;
    }
  }
}

void Matrix_d::SetDiagonal(double NewValue)
{
#ifdef BOUNDS_CHECK
  assert(GetNumberOfRows()==GetNumberOfCols());
#endif
  for (int i=0; i<GetNumberOfRows(); i++){
    (*this)(i,i) = NewValue;
  }
}

void Matrix_d::Load(const char *FileName)
{
  FileName >> *this;
}

ostream& operator<< (ostream& os,const Matrix_d& M)
{
  int i;
  int j;

  os.setf(ios::fixed);

  for (i=0; i<M.GetNumberOfRows(); i++){
    for (j=0; j<M.GetNumberOfCols(); j++){
      if (M.IsMissingValue(i,j)){
	os << setw( 11 ) << "#" << " ";
      } else {
	os << setprecision( 6 ) << setw( 11 ) << M(i,j) << " ";
      }
    }
    os << endl;
  }
  return os;
}	

void operator>> ( const char * FileName, Matrix_d& M )
{
  int NumRows = 0;
  int NumCols = 0;
  int NumMissingValues = 0;
  int PrevNumCols = 0;
  int MissingElementsThisRow = 0;
  int CurrentMissingValue = 0;
  ifstream fileOne;
  ifstream fileTwo;
  Vector_d CurrentRow;

  M._MissingRow.clear();
  M._MissingCol.clear();

  fileOne.open(FileName);
  assert(fileOne.is_open());
  while( !fileOne.eof() ){
    fileOne >> CurrentRow;
    
    NumMissingValues += CurrentRow.GetNumberOfMissingValues();
    NumCols = CurrentRow.GetNumberOfElements();
    
#ifdef BOUNDS_CHECK
    // Check for ragged input
    assert(NumRows==0 || NumCols==PrevNumCols);
#endif
    PrevNumCols = NumCols;
    
    NumRows++;
  }
  fileOne.close();
  
  M.SetNumberOfElements(NumRows,NumCols);
  M._MissingRow.assign(NumMissingValues,0);
  M._MissingCol.assign(NumMissingValues,0);

  // Rewind to beginning of file and
  // reset eof flag
   
   
  fileTwo.open(FileName);
   
  // Load matrix elements

  CurrentMissingValue = 0;
  for (int i = 0; i < M.GetNumberOfRows(); i++ ){
    fileTwo >> CurrentRow;
    M.SetRow( i, CurrentRow );
    MissingElementsThisRow = CurrentRow.GetNumberOfMissingValues();
    
    // If there are missing elements store their row and column indices
    
    if ( MissingElementsThisRow ){
      for (int j = 0; j < CurrentRow.GetNumberOfElements(); j++ ){
	if ( CurrentRow.IsMissingElement( j ) ){
	  M._MissingRow[CurrentMissingValue] = i;
	  M._MissingCol[CurrentMissingValue] = j;
	  CurrentMissingValue++;
	}
      }
    }
  }
  
  fileTwo.close();
  return;
}	

double Matrix_d::Determinant() const
{
  int sign;
  double LogDeterminant;

  LogDeterminant = this->LogDeterminant( &sign );

  return( sign * exp( LogDeterminant ) );
}

double Matrix_d::LogDeterminant( int *sign ) const
{
  // Perform lower-upper decomposition on a copy of this matrix
  Matrix_d temp = (*this);
  gsl_permutation *permutation = gsl_permutation_calloc(GetNumberOfRows());
  *sign = temp.LU_decomp(permutation);
  gsl_permutation_free( permutation );

  // return sum of logarithms of the diagonal elements
  return temp.LU_lndet();
}

void Matrix_d::InvertUsingLUDecomposition()
{
  int ReturnValue;
  int sign;
  gsl_matrix *LowerUpper;
  gsl_matrix *Inverse;
  gsl_permutation *permutation;

  // Copy matrix into lu because gsl_la_decomp_LU_impl destroys the
  // matrix it is sent. GSL checks whether matrix is square etc.
  // Can't use gsl_matrix_copy because matrix is possibly double and
  // gsl_matrix copy assumes that it's double.
  Inverse = gsl_matrix_calloc(GetNumberOfRows(),GetNumberOfCols());
  LowerUpper = gsl_matrix_calloc(GetNumberOfRows(),GetNumberOfCols());
  permutation = gsl_permutation_calloc(GetNumberOfRows());
  for (int i=0; i<GetNumberOfRows(); i++){
    for (int j=0; j<GetNumberOfCols(); j++){
      int offset = i * Inverse->size2 + j;
      Inverse->data[offset] = _matrix->data[offset];
      LowerUpper->data[offset] = _matrix->data[offset];
    }
  }

  // Perform lower-upper decomposition
  ReturnValue = gsl_linalg_LU_decomp( LowerUpper, permutation, &sign );
  assert(ReturnValue==GSL_SUCCESS);

  // Calculate inverse using LU decomposed form of matrix
  gsl_linalg_LU_invert( LowerUpper, permutation, Inverse );

  // Copy results from Inverse back into our double matrix[][]
  for (int i=0; i<GetNumberOfRows(); i++){
    for (int j=0; j<GetNumberOfCols(); j++){
      int offset = i * Inverse->size2 + j;
      _matrix->data[offset] = static_cast<double>(Inverse->data[offset]);
    }
  }

  // Free memory
  gsl_permutation_free( permutation );
  gsl_matrix_free( LowerUpper );
  gsl_matrix_free( Inverse );
}

Vector_d Matrix_d::GetRow( int row ) const
{
  Vector_d temp(GetNumberOfCols());
  for(int col=0; col<GetNumberOfCols(); col++){
    temp(col) = (*this)(row,col);
  }
  return temp;
}

Vector_d Matrix_d::GetColumn( int col ) const
{
  Vector_d temp(GetNumberOfRows());
  for(int row=0; row<GetNumberOfRows(); row++){
    temp(row) = (*this)(row,col);
  }
  return( temp );
}

void Matrix_d::SetRow(int row,const Vector_d& v)
{
#ifdef BOUNDS_CHECK
  assert(GetNumberOfCols() == v.GetNumberOfElements());
  assert(row<GetNumberOfRows());
#endif   
  for (int i=0;i<GetNumberOfCols();i++){
    (*this)(row,i) = v(i);
  }
}

void Matrix_d::SetColumn(int col,const Vector_d& v)
{
#ifdef BOUNDS_CHECK
  assert(GetNumberOfRows() == v.GetNumberOfElements());
  assert(col<GetNumberOfCols());
#endif   
  for (int i=0;i<GetNumberOfRows();i++){
    (*this)(i,col) = v(i);
  }
}

void Matrix_d::Symmetrize()
{
  double m;

#ifdef BOUNDS_CHECK
  assert(GetNumberOfRows() == GetNumberOfCols());
#endif   
  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      for (int j = i + 1; j < GetNumberOfCols(); j++ )
	{
	  m = (*this)(i,j) + (*this)(j,i);
	  m /= 2;
	  (*this)(i,j) = (*this)(j,i) = m;
	}
    }
}

Vector_d Matrix_d::GetDiagonal() const
{
  Vector_d temp( GetNumberOfRows() );

#ifdef BOUNDS_CHECK
  assert(GetNumberOfRows() == GetNumberOfCols());
#endif   

  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      temp(i) = (*this)(i,i);
    }
  return( temp );
}

Matrix_d Matrix_d::SubMatrix( int rowstart, int rowfinish, int colstart, int colfinish ) const
{
#ifdef BOUNDS_CHECK
  assert(rowstart < GetNumberOfRows());
  assert(rowfinish < GetNumberOfRows());
  assert(rowstart >= 0);
  assert(rowfinish >= 0);
  assert(colstart < GetNumberOfCols());
  assert(colfinish < GetNumberOfCols());
  assert(colstart >= 0);
  assert(colfinish >= 0);
  assert(rowstart <= rowfinish);
  assert(colstart <= colfinish);
#endif

  Matrix_d temp( rowfinish - rowstart + 1, colfinish - colstart + 1 );

  for ( int row = rowstart; row <= rowfinish; row++ ){
    for ( int col = colstart; col <= colfinish; col++ ){
      temp( row - rowstart, col - colstart ) = (*this)(row,col);
    }
  }
  return( temp );
}

void Matrix_d::SubMatrix2( int rowstart, int rowfinish, int colstart, int colfinish )
{
#ifdef BOUNDS_CHECK
  assert(rowstart < GetNumberOfRows());
  assert(rowfinish < GetNumberOfRows());
  assert(rowstart >= 0);
  assert(rowfinish >= 0);
  assert(colstart < GetNumberOfCols());
  assert(colfinish < GetNumberOfCols());
  assert(colstart >= 0);
  assert(colfinish >= 0);
  assert(rowstart <= rowfinish);
  assert(colstart <= colfinish);
#endif
  Matrix_d temp;
  temp.SetNumberOfElements(rowfinish - rowstart + 1, colfinish - colstart + 1 );
  int count = 0, count2 = 0;
  Vector_d rowmissing( GetNumberOfMissingValues() ), colmissing( GetNumberOfMissingValues() );

  for ( int row = rowstart; row <= rowfinish; row++ ){
    for ( int col = colstart; col <= colfinish; col++ ){
      temp( row - rowstart, col - colstart ) = (*this)(row,col);
      if( count2 < GetNumberOfMissingValues() ){
	while( (int)_MissingRow[ count2 ] < row || 
	       ( (int)_MissingRow[ count2 ] == row && (int)_MissingCol[ count2 ] < col ) )
	  count2++;
	if( (int)_MissingRow[ count2 ] == row && (int)_MissingCol[ count2 ] == col ){
	  rowmissing( count ) = row - rowstart;
	  colmissing( count ) = col - colstart;
	  count++;
	  count2++;
	}
      }
    }
  }

  (*this) = temp;

  _MissingRow.assign(count,0);
  _MissingCol.assign(count,0);
  // Copy the missing value row and column indices if necessary
  for ( int i = 0; i < GetNumberOfMissingValues(); i++ ){
    (int)_MissingRow[i] = rowmissing(i);
    (int)_MissingCol[i] = colmissing(i);
  }

}

Matrix_d ConcatenateHorizontally( const Matrix_d &m1, const Matrix_d &m2 )
{
#ifdef BOUNDS_CHECK
  assert(m1.GetNumberOfRows()==m2.GetNumberOfRows());
#endif

  Matrix_d temp( m1.GetNumberOfRows(), m1.GetNumberOfCols() + m2.GetNumberOfCols() );

  for (int col = 0; col < m1.GetNumberOfCols(); col++ )
    temp.SetColumn( col, m1.GetColumn( col ) );
  for (int col = 0; col < m2.GetNumberOfCols(); col++ )
    temp.SetColumn( col + m1.GetNumberOfCols(), m2.GetColumn( col ) );

  return( temp );
}

Matrix_d Matrix_d::operator+( const Matrix &m ) const
{
  Matrix_d temp( GetNumberOfRows(), GetNumberOfCols() );

  assert( GetNumberOfRows() == m.GetNumberOfRows() && GetNumberOfCols() == m.GetNumberOfCols() );

  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      for (int j = 0; j < GetNumberOfCols(); j++ )
	{
	  temp(i,j) = (*this)(i,j) + (double)m(i,j);
	}
    }

  return( temp );
}

Matrix_d Matrix_d::operator+( const Matrix_d &m ) const
{
  Matrix_d temp( GetNumberOfRows(), GetNumberOfCols() );

  assert( GetNumberOfRows() == m.GetNumberOfRows() && GetNumberOfCols() == m.GetNumberOfCols() );

  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      for (int j = 0; j < GetNumberOfCols(); j++ )
	{
	  temp(i,j) = (*this)(i,j) + (double)m(i,j);
	}
    }

  return( temp );
}

Matrix_d Matrix_d::operator+( const Matrix_i &m ) const
{
  Matrix_d temp( GetNumberOfRows(), GetNumberOfCols() );

  assert( GetNumberOfRows() == m.GetNumberOfRows() && GetNumberOfCols() == m.GetNumberOfCols() );

  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      for (int j = 0; j < GetNumberOfCols(); j++ )
	{
	  temp(i,j) = (*this)(i,j) + (double)m(i,j);
	}
    }

  return( temp );
}

Matrix_d Matrix_d::operator+( double f ) const
{
  Matrix_d temp( GetNumberOfRows(), GetNumberOfCols() );

  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      for (int j = 0; j < GetNumberOfCols(); j++ )
	{
	  temp(i,j) = (*this)(i,j) + f;
	}
    }

  return( temp );
}

Matrix_d &Matrix_d::operator+=( const Matrix_d &m )
{
  assert( GetNumberOfRows() == m.GetNumberOfRows() && GetNumberOfCols() == m.GetNumberOfCols() );

  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      for (int j = 0; j < GetNumberOfCols(); j++ )
	{
	  (*this)(i,j) += m(i,j);
	}
    }

  return( *this );
}

Matrix_d &Matrix_d::operator*=( const double f )
{
  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      for (int j = 0; j < GetNumberOfCols(); j++ )
	{
	  (*this)(i,j) *= f;
	}
    }

  return( *this );
}

Matrix_d Matrix_d::operator-( const Matrix_d &m ) const
{
  Matrix_d temp( GetNumberOfRows(), GetNumberOfCols() );

  assert( GetNumberOfRows() == m.GetNumberOfRows() && GetNumberOfCols() == m.GetNumberOfCols() );

  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      for (int j = 0; j < GetNumberOfCols(); j++ )
	{
	  temp(i,j) = (*this)(i,j) - m(i,j);
	}
    }

  return( temp );
}

Matrix_d Matrix_d::operator*( const Matrix_d &m ) const
{
  Matrix_d temp( GetNumberOfRows(), m.GetNumberOfCols() );

#ifdef BOUNDS_CHECK
  assert(GetNumberOfCols()==m.GetNumberOfRows());
#endif

  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      for (int j = 0; j < m.GetNumberOfCols(); j++ )
	{
	  double d = (double)0.0;
	  for (int k = 0; k < m.GetNumberOfRows(); k++ )
	    d += (*this)(i,k) * m(k,j);
	  temp(i,j) = d;
	}
    }

  return( temp );
}

Matrix_d Matrix_d::operator*( double f ) const
{
  Matrix_d temp( GetNumberOfRows(), GetNumberOfCols() );

  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      for (int j = 0; j < GetNumberOfCols(); j++ )
	{
	  temp(i,j) = (*this)(i,j) * f;
	}
    }

  return( temp );
}

Matrix_d operator*( double f, const Matrix_d &m )
{
  Matrix_d temp( m.GetNumberOfRows(), m.GetNumberOfCols() );

  for (int i = 0; i < m.GetNumberOfRows(); i++ )
    {
      for (int j = 0; j < m.GetNumberOfCols(); j++ )
	{
	  temp(i,j) = m(i,j) * f;
	}
    }

  return( temp );
}

Matrix_d Matrix_d::operator/( double f ) const
{
  Matrix_d temp( GetNumberOfRows(), GetNumberOfCols() );

  assert(f != 0.0);
  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      for (int j = 0; j < GetNumberOfCols(); j++ )
	{
	  temp(i,j) = (*this)(i,j) / f;
	}
    }

  return( temp );
}

Matrix_d &Matrix_d::operator/=( double f )
{
  assert(f != 0.0);
  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      for (int j = 0; j < GetNumberOfCols(); j++ )
	{
	  (*this)(i,j) /= f;
	}
    }

  return( *this );
}

int Matrix_d::GetMissingValueRow( int index ) const
{
#ifdef BOUNDS_CHECK
  assert(index < GetNumberOfMissingValues());
  assert(index >= 0);
#endif
  return( _MissingRow[index] );
}

int Matrix_d::GetMissingValueCol( int index ) const
{
#ifdef BOUNDS_CHECK
  assert(index < GetNumberOfMissingValues());
  assert(index >= 0);
#endif
  return( _MissingCol[index] );
}

int Matrix_d::GetNumberOfMissingValues() const
{
  return _MissingRow.size();
}

bool Matrix_d::IsMissingValue( int row, int col ) const
{
#ifdef BOUNDS_CHECK
  assert(row < GetNumberOfRows());
  assert(row >= 0);
  assert(col < GetNumberOfCols());
  assert(col >= 0);
#endif
  for (int i = 0; i < GetNumberOfMissingValues(); i++ ){
    if ( row == (int)_MissingRow[i] && col == (int)_MissingCol[i] ){
      return true;
    }
  }
  return false;
}

Vector_d Matrix_d::ColumnSumExcludingMissingValues() const
{
  Vector_d temp( GetNumberOfCols() );

  temp = ColumnSum();
  for (int i = 0; i < GetNumberOfMissingValues(); i++ ){
    temp( _MissingCol[i] ) -= (*this)(_MissingRow[i],_MissingCol[i]);
  }
  return( temp );
}

Vector_d Matrix_d::ColumnMeanExcludingMissingValues() const
{
  Vector_d NonMissingValueCount( GetNumberOfCols() );

  // Count non-missing values for each column

  NonMissingValueCount.SetElements( (double)GetNumberOfRows() );
  for (int i = 0; i < GetNumberOfMissingValues(); i++ )
    {
      NonMissingValueCount( _MissingCol[i] )--;
    }
  // Check whether all values in a column are missing to avoid
  // division by zero

#ifdef BOUNDS_CHECK
  for (int i = 0; i < GetNumberOfCols(); i++ ){
    assert(NonMissingValueCount( i ) != 0.0);
  }
#endif
  return( ColumnSumExcludingMissingValues() / NonMissingValueCount );
}

void Matrix_d::SetMissingValuesToColumnMeans()
{
  Vector_d MissingValueEstimate = ColumnMeanExcludingMissingValues();

  for (int i = 0; i < GetNumberOfMissingValues(); i++ ){
    (*this)(_MissingRow[i],_MissingCol[i]) = MissingValueEstimate( _MissingCol[i] );
  }
}

Matrix_d Matrix_d::Transpose() const
{
  Matrix_d temp( GetNumberOfCols(), GetNumberOfRows() );

  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      for (int j = 0; j < GetNumberOfCols(); j++ )
	{
	  temp(j,i) = (*this)(i,j);
	}
    }

  return( temp );
}

int Matrix_d::CholeskyDecomposition( Matrix_d *CD ) const
{
  double sum;
  double **a;
  Vector_d p;

  assert(GetNumberOfRows() == GetNumberOfCols());
  assert(IsSymmetric());

  // Construct Cholesky factor matrix

  CD->SetNumberOfElements(GetNumberOfRows(),GetNumberOfRows());

  // Allocate temporary double working matrix and copy contents of matrix[][]

  a = new double * [GetNumberOfRows()];
  for (int i = 0; i < GetNumberOfRows(); i++ )
    a[i] = new double[GetNumberOfCols()];
  for (int i = 0; i < GetNumberOfRows(); i++ )
    for (int j = 0; j < GetNumberOfCols(); j++ )
      a[i][j] = (*this)(i,j);

  // Construct Vector_d for Cholesky diagonal

  p.SetNumberOfElements( GetNumberOfRows() );

  int k;
  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      for (int j = i; j < GetNumberOfRows(); j++ )
	{
	  for ( sum = a[i][j], k = ((int)i)-1; k >= 0; k-- )
            sum -= a[i][k] * a[j][k];
	  if ( i == j )
	    {
	      if ( sum <= 0.0 )
		{
		  if ( fabs((double)sum ) < 1.0e-6 )
		    sum *= (double)-1.0;
		  else
		    {
		      for ( i = 0; i < GetNumberOfRows(); i++ )
			delete a[i];
		      delete [] a;
		      return( 0 );
		    }
		}
	      p( i ) = (double) sqrt((double)sum );
	    }
	  else
            if ( p( i ) != 0.0 )
	      a[j][i] = sum / p( i );
	}
    }

  for (int i = 0; i < GetNumberOfRows(); i++ ){
    for (int j = 0; j < GetNumberOfCols(); j++ ){
      if (i==j){
	(*CD)(i,i) = p(i);
      } else if (i>j) {
	(*CD)(i,j) = a[i][j];
      } else {
	(*CD)(i,j) = static_cast<double>(0.0);
      }
    }
  }
  // Free working matrix a[][]
   
  for (int i = 0; i < GetNumberOfRows(); i++ )
    delete a[i];
  delete [] a;

  return( 1 );
}

Vector_d Matrix_d::ColumnSum() const
{
  Vector_d temp( GetNumberOfCols() );

  temp.SetElements( (double)0.0 );
  for (int i = 0; i < GetNumberOfRows(); i++ )
    for (int j = 0; j < GetNumberOfCols(); j++ )
      temp(j) += (*this)(i,j);

  return( temp );
}

Vector_d Matrix_d::RowSum() const
{
  Vector_d temp( GetNumberOfRows() );

  temp.SetElements( (double)0.0 );
  for (int i = 0; i < GetNumberOfCols(); i++ )
    for (int j = 0; j < GetNumberOfRows(); j++ )
      temp(j) += (*this)(j,i);

  return( temp );
}

Vector_d Matrix_d::ColumnMean() const
{
  return( ColumnSum() / GetNumberOfRows() );
}

Vector_d Matrix_d::RowMean() const
{
  return( RowSum() / GetNumberOfCols() );
}

bool Matrix_d::IsSymmetric() const
{
  for (int i = 0; i < GetNumberOfRows(); i++ ){
    for (int j = i+1; j < GetNumberOfCols(); j++ ){
      if ((*this)(i,j) != (*this)(j,i)){
	return false;
      }
    }
  }
  return true;
}

void Matrix_d::Eigenvalue2( Vector_d *Eigenvalue )
{
  gsl_matrix *GSLData;
  gsl_vector *GSLVal;
  gsl_eigen_symm_workspace *w;

  GSLData = gsl_matrix_calloc( GetNumberOfRows(), GetNumberOfCols() );
  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      for (int j = 0; j < GetNumberOfCols(); j++ )
	{
	  int offset = i * GSLData->size2 + j;
	  GSLData->data[offset] = _matrix->data[offset];
	}
    }
  GSLVal = gsl_vector_calloc( GetNumberOfRows() );
  w = gsl_eigen_symm_alloc( GetNumberOfRows() );
  
  assert(gsl_eigen_symm( GSLData, GSLVal, w ) == GSL_SUCCESS);
  
  Eigenvalue->SetNumberOfElements( GetNumberOfRows() );
  for(int i = 0; i < GetNumberOfRows(); i++ )
    {
      (*Eigenvalue)(i) = ((double)(GSLVal->data[i * GSLVal->stride]));
    }

  gsl_eigen_symm_free( w ); 
  gsl_vector_free( GSLVal );
  gsl_matrix_free( GSLData );

  return;
}

// This function finds the indexes of the elements
// of Vector_d whose values are equal to t
// Returns a two-column matrix, where the first column
// stores the rows, the second the columns.
Matrix_d Matrix_d::FindElement(double t)
{
  int n = 0;
  Matrix_d Index;
  Matrix_d Tmp(GetNumberOfRows()*GetNumberOfCols(), 2);

  Tmp.SetElements((double)0.0);
   
  for (int i = 0; i < GetNumberOfRows(); i++ )
    for (int j = 0; j < GetNumberOfCols(); j++ ){
      if ( (*this)(i,j) == t ){
	Tmp(n,0) = i;
	Tmp(n,1) = j;
	n++;
      }
    }

  Index.SetNumberOfElements(n,2);

  for (int i = 0; i < n; i++ ){
    Index(i,0) = Tmp(i,0);
    Index(i,1) = Tmp(i,1);
  }

  return(Index);
}

/**
 * Computes the logarithm of the absolute value of the determinant of 
 * a matrix from its LU decomposition (*this). This function may be
 * useful is the direct computation of the determinant would underflow
 * or overflow.
 */
double
Matrix_d::LU_lndet()
{
  double lndet = 0.0;
  for (int i=0; i<GetNumberOfRows(); i++){
    lndet += log (fabs(static_cast<double>((*this)(i,i))));
  }
  return lndet;
}

/**
 * Factorizes the square matrix into the LU decomposition, given a 
 * permutation matrix.
 */
int
Matrix_d::LU_decomp (gsl_permutation * p){
  assert(GetNumberOfRows() == GetNumberOfCols());
  assert((int)p->size == GetNumberOfRows());

  int signum = 1;
  gsl_permutation_init (p);

  for (int j=0; j < GetNumberOfRows()-1; j++){
    /* Find maximum in the j-th column */

    double ajj, max = static_cast<double>(fabs(static_cast<double>((*this)(j,j))));
    size_t i_pivot = j;

    for (int i = j + 1; i < GetNumberOfRows(); i++){
      double aij = static_cast<double>(fabs(static_cast<double>((*this)(i,j))));

      if (aij > max){
	max = aij;
	i_pivot = i;
      }
    }

    if ((int)i_pivot != j){
      gsl_matrix_swap_rows (_matrix, j, i_pivot);
      gsl_permutation_swap (p, j, i_pivot);
      signum = -signum;
    }

    ajj = (*this)(j,j);

    if (ajj != 0.0){
      for (int i = j + 1; i < GetNumberOfRows(); i++){
	double aij = (*this)(i,j) / ajj;
	(*this)(i,j) = aij;

	for (int k = j + 1; k < GetNumberOfRows(); k++){
	  double aik = (*this)(i,k);
	  double ajk = (*this)(j,k);
	  (*this)(i,k) = aik - aij * ajk;
	}
      }
    }
  }
      
  return signum;
}

void Matrix_d::SetMissingElement(int row, int col)
{
    if (!IsMissingValue(row, col)) {
        _MissingRow.push_back(row);
        _MissingCol.push_back(col);
    }
}
