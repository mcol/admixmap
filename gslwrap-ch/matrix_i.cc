...
#define MATRIX_TYPE_i
#define BOUNDS_CHECK 1

using namespace std;

Matrix_i::Matrix_i():
  _matrix((gsl_matrix_int*)malloc(sizeof(gsl_matrix_int)))
{
  assert(_matrix);
  _matrix->size1 = 0;
  _matrix->size2 = 0;
...
Matrix_i::Matrix_i(size_t NewRows, size_t NewCols):
  _matrix(gsl_matrix_int_calloc(NewRows,NewCols))
{
  assert(NewRows != 0);
  assert(NewCols != 0);
  assert(_matrix);
...
Matrix_i::Matrix_i(const Matrix_i& other):
  _matrix(gsl_matrix_int_calloc(other.GetNumberOfRows(),other.GetNumberOfCols()))
{
  _MissingRow = other._MissingRow;
  _MissingCol = other._MissingCol;
  gsl_matrix_int_memcpy(_matrix,other._matrix);
}
...
Matrix_i &Matrix_i::operator=( const Matrix& m )
{
  if((void*)this == (void*)&m){
    return *this;
  }
  ...
  if (GetNumberOfRows() != m.GetNumberOfRows() ||
    ...
    gsl_matrix_int_free( _matrix );
    _matrix = gsl_matrix_int_calloc(m.GetNumberOfRows(),m.GetNumberOfCols());
  }
  ...
  for(int i=0; i<GetNumberOfRows(); i++){
    for(int j=0; j<GetNumberOfCols(); j++){
      (*this)(i,j) = static_cast<int>(m(i,j));
    }
  }
...
Matrix_i &Matrix_i::operator=( const Matrix_d& m )
{
  if((void*)this == (void*)&m){
    return *this;
  }
...
Matrix_i &Matrix_i::operator=( const Matrix_i& m )
{
  if((void*)this == (void*)&m){
    return *this;
  }
...
int&
Matrix_i::operator()( int row, int col )
{
  return _matrix->data[row * GetNumberOfCols() + col];
}

int&
Matrix_i::operator()( int row, int col ) const
{
  return _matrix->data[row * GetNumberOfCols() + col];
}

Matrix_d Matrix_i::Double() const
{
...
Matrix_i Matrix_i::Integer() const
{
...
int Matrix_i::GetNumberOfRows() const
{
  return _matrix->size1;
}

int Matrix_i::GetNumberOfCols() const
{
  return _matrix->size2;
}

void Matrix_i::SetNumberOfElements(size_t NewRows, size_t NewCols)
{
  _MissingRow.clear();
  _MissingCol.clear();
  ...
  if(NewRows==0 || NewCols==0){
    cerr << endl << endl << endl;
    cerr << "BUG - Matrix_i::SetNumberOfElements(size_t,size_t) was passed zero as an argument... but continuing" << endl;
  ...
  if (GetNumberOfRows() != (int)NewRows ||
    ...
    SetElements(static_cast<int>(0.0));
  }
}

void Matrix_i::SetElements(int NewValue)
{
  for (int i=0; i<GetNumberOfRows(); i++){
    for (int j=0; j<GetNumberOfCols(); j++){
      (*this)(i,j) = NewValue;
...
void Matrix_i::SetDiagonal(int NewValue)
{
#ifdef BOUNDS_CHECK
...
void Matrix_i::Load(const char *FileName)
{
  FileName >> *this;
}

ostream& operator<< (ostream& os,const Matrix_i& M)
{
  int i;
  int j;
...
void operator>> ( const char * FileName, Matrix_i& M )
{
  int NumRows = 0;
  int NumCols = 0;
...
double Matrix_i::Determinant() const
{
  int sign;
...
double Matrix_i::LogDeterminant( int *sign ) const
{
...
void Matrix_i::InvertUsingLUDecomposition()
{
  int ReturnValue;
  int sign;
  ...
  // Copy results from Inverse back into our int matrix[][]
  for (int i=0; i<GetNumberOfRows(); i++){
    for (int j=0; j<GetNumberOfCols(); j++){
      int offset = i * Inverse->size2 + j;
      _matrix->data[offset] = static_cast<int>(Inverse->data[offset]);
    }
  }
...
Vector_i Matrix_i::GetRow( int row ) const
{
  Vector_i temp(GetNumberOfCols());
...
Vector_i Matrix_i::GetColumn( int col ) const
{
  Vector_i temp(GetNumberOfRows());
...
void Matrix_i::SetRow(int row,const Vector_i& v)
{
#ifdef BOUNDS_CHECK
...
void Matrix_i::SetColumn(int col,const Vector_i& v)
{
#ifdef BOUNDS_CHECK
...
Vector_i Matrix_i::GetDiagonal() const
{
  Vector_i temp( GetNumberOfRows() );
...
Matrix_i Matrix_i::SubMatrix( int rowstart, int rowfinish, int colstart, int colfinish ) const
{
#ifdef BOUNDS_CHECK
  ...
  Matrix_i temp( rowfinish - rowstart + 1, colfinish - colstart + 1 );
...
void Matrix_i::SubMatrix2( int rowstart, int rowfinish, int colstart, int colfinish )
{
#ifdef BOUNDS_CHECK
  assert(rowstart < GetNumberOfRows());
  assert(rowfinish < GetNumberOfRows());
  ...
  Vector_i rowmissing( GetNumberOfMissingValues() ), colmissing( GetNumberOfMissingValues() );
...
Matrix_i ConcatenateHorizontally( const Matrix_i &m1, const Matrix_i &m2 )
{
#ifdef BOUNDS_CHECK
  assert(m1.GetNumberOfRows()==m2.GetNumberOfRows());
#endif

  Matrix_i temp( m1.GetNumberOfRows(), m1.GetNumberOfCols() + m2.GetNumberOfCols() );
...
Matrix_i Matrix_i::operator+( const Matrix &m ) const
{
  Matrix_i temp( GetNumberOfRows(), GetNumberOfCols() );
...
Matrix_i Matrix_i::operator+( const Matrix_d &m ) const
{
  Matrix_i temp( GetNumberOfRows(), GetNumberOfCols() );
...
Matrix_i Matrix_i::operator+( const Matrix_i &m ) const
{
  Matrix_i temp( GetNumberOfRows(), GetNumberOfCols() );
...
Matrix_i Matrix_i::operator+( int f ) const
{
  Matrix_i temp( GetNumberOfRows(), GetNumberOfCols() );
...
Matrix_i &Matrix_i::operator*=( const int f )
{
  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
      for (int j = 0; j < GetNumberOfCols(); j++ )
...
Matrix_i Matrix_i::operator*( const Matrix_i &m ) const
{
  Matrix_i temp( GetNumberOfRows(), m.GetNumberOfCols() );
...
Matrix_i operator*( int f, const Matrix_i &m )
{
  Matrix_i temp( m.GetNumberOfRows(), m.GetNumberOfCols() );
...
Matrix_i &Matrix_i::operator/=( int f )
{
  assert(f != 0.0);
  for (int i = 0; i < GetNumberOfRows(); i++ )
    {
...
int Matrix_i::GetMissingValueRow( int index ) const
{
#ifdef BOUNDS_CHECK
...
int Matrix_i::GetMissingValueCol( int index ) const
{
#ifdef BOUNDS_CHECK
  assert(index < GetNumberOfMissingValues());
  assert(index >= 0);
...
int Matrix_i::GetNumberOfMissingValues() const
{
  return _MissingRow.size();
}

bool Matrix_i::IsMissingValue( int row, int col ) const
{
#ifdef BOUNDS_CHECK
  assert(row < GetNumberOfRows());
  assert(row >= 0);
...
Vector_i Matrix_i::ColumnSumExcludingMissingValues() const
{
  Vector_i temp( GetNumberOfCols() );
...
Vector_i Matrix_i::ColumnMeanExcludingMissingValues() const
{
  Vector_i NonMissingValueCount( GetNumberOfCols() );

  // Count non-missing values for each column

  NonMissingValueCount.SetElements( (int)GetNumberOfRows() );
  for (int i = 0; i < GetNumberOfMissingValues(); i++ )
    {
...
void Matrix_i::SetMissingValuesToColumnMeans()
{
  Vector_i MissingValueEstimate = ColumnMeanExcludingMissingValues();
...
{
  Matrix_i temp( GetNumberOfCols(), GetNumberOfRows() );
...
int Matrix_i::CholeskyDecomposition( Matrix_i *CD ) const
{
  int sum;
  int **a;
  Vector_i p;
  ...
  // Allocate temporary int working matrix and copy contents of matrix[][]

  a = new int * [GetNumberOfRows()];
  for (int i = 0; i < GetNumberOfRows(); i++ )
    a[i] = new int[GetNumberOfCols()];
  for (int i = 0; i < GetNumberOfRows(); i++ )
    for (int j = 0; j < GetNumberOfCols(); j++ )
      a[i][j] = (*this)(i,j);

  // Construct Vector_i for Cholesky diagonal
...
Vector_i Matrix_i::ColumnSum() const
{
  Vector_i temp( GetNumberOfCols() );

  temp.SetElements( (int)0.0 );
  for (int i = 0; i < GetNumberOfRows(); i++ )
    for (int j = 0; j < GetNumberOfCols(); j++ )
      temp(j) += (*this)(i,j);
...
Vector_i Matrix_i::RowSum() const
{
  Vector_i temp( GetNumberOfRows() );
...
Vector_i Matrix_i::ColumnMean() const
{
  return( ColumnSum() / GetNumberOfRows() );
}

Vector_i Matrix_i::RowMean() const
{
  return( RowSum() / GetNumberOfCols() );
}

bool Matrix_i::IsSymmetric() const
{
  for (int i = 0; i < GetNumberOfRows(); i++ ){
    for (int j = i+1; j < GetNumberOfCols(); j++ ){
...
void Matrix_i::Eigenvalue2( Vector_i *Eigenvalue )
{
  gsl_matrix *GSLData;
  gsl_vector *GSLVal;
  ...
  for(int i = 0; i < GetNumberOfRows(); i++ )
    {
      (*Eigenvalue)(i) = ((int)(GSLVal->data[i * GSLVal->stride]));
    }
...
// of Vector_i whose values are equal to t
// Returns a two-column matrix, where the first column
// stores the rows, the second the columns.
Matrix_i Matrix_i::FindElement(int t)
{
  int n = 0;
  Matrix_i Index;
  Matrix_i Tmp(GetNumberOfRows()*GetNumberOfCols(), 2);

  Tmp.SetElements((int)0.0);
 ...
 */
int
Matrix_i::LU_decomp (gsl_permutation * p){
  assert(GetNumberOfRows() == GetNumberOfCols());
  ...
  for (int j=0; j < GetNumberOfRows()-1; j++){
    ...
    int ajj, max = static_cast<int>(fabs(static_cast<double>((*this)(j,j))));
    size_t i_pivot = j;

    for (int i = j + 1; i < GetNumberOfRows(); i++){
      int aij = static_cast<int>(fabs(static_cast<double>((*this)(i,j))));
    ...
    if ((int)i_pivot != j){
      gsl_matrix_int_swap_rows (_matrix, j, i_pivot);
...
void Matrix_i::SetMissingElement(int row, int col)
{
