/*
  prog to convert hapmap data (for a single chromosome) to ADMIXMAP format
*/
...
int main(){
  ...
  while (! genotypesin.eof() ){
    ...
    if(locus==0) locusfile << "\t#";
      ...
      if(position - prev < 0) {
	prev = position - 1;
	cout << "\nnegative distance set to 1 bp at locus " << locus << endl;
      }
      locusfile << 0.0000013 * (double)(position-prev);
    }
  ...
  cout << "\nFinished writing locusfile. " << NUMLOCI <<  " loci" << endl;
   ...
   for(indiv = 0; indiv < NUMIND; ++indiv) if(indisvalid[indiv]){
     ++NUMVALIDINDS;
     cout << "Writing genotypes for individual " << indiv << "\r" << flush; 
     // INDIVID[indiv] << " "  << indisvalid[indiv] << endl << flush;
