...
## rearrange genotypes in STRUCTURE format
str.genotypes <- matrix(data=NA, nrow=2*N, ncol=1+L)
for(ind in 1:N) {
  str.genotypes[ind*2 -1, ] <- c(id[ind], gsub(",[0-9]", "", genotypes[ind,]))
  str.genotypes[ind*2, ]    <-  c(id[ind], gsub("[0-9],", "", genotypes[ind,]))
}
str.row1 <- c("", paste("X", seq(1:L), sep=""))
x.str <- 100*x
x.str[is.na(x.str)] <- -1
str.row2 <- c(" ", as.character(x.str))
str.table <- rbind(str.row1, str.row2, str.genotypes)
write.table(str.table, file="strdata.txt", sep=" ", quote=F, row.names=F, col.names=F)
