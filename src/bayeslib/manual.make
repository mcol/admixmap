###############################################################################################
# makefile for compiling bayeslib
#
...
# name of compiled library: LIBNAME
###############################################################################################
...
## DO NOT EDIT BELOW HERE ##
SUBLIBS = libsampler libutils libbayesreg

export CC PCC CPPFLAGS SERIAL_INCLUDES PARALLEL_INCLUDES


all:	$(SUBLIBS) bayeslib
...
dist:
	rm bayeslib.tar.gz
	mkdir bayeslib
	cp *.cc *.h *.txt README bayeslib
	cp makefile bayeslib
	tar -czf bayeslib.tar.gz bayeslib
	rm -R bayeslib
