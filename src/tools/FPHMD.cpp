 ...
 * NB: input data files must be named
 * "chr#_phased.txt",
 * "chr#_sample.txt" and
 * "chr#_legend.txt",
 *
...
void PrintHelpText(char **argv)
{
  cout <<
      ...
      << "Usage: " << argv[0] << " "
      << "-c <chrm> [-p <prefix>] [-g <genotypesfile>]" << endl
      << " [-l <locusfile>] [-n <numloci>]" << endl
      << "where <prefix> is the directory containing data files ('Eur', 'Afr' or 'Asian')," << endl
...
int main(int argc, char **argv)
{
  ...
  while ((ich = getopt(argc, argv, "hc:g:l:p:n:qv")) != EOF) {
    ...
    case 'n':{                 //upper limit on number of loci
