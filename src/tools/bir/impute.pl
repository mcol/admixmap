...
for (my $i=0; $i < 3; ++$i) {
    ...
    system("impute -h $datadir/haplo.txt -l $datadir/legend.txt -g $datadir/geno.txt -m $datadir/map.txt -o $resultsdir/impgenotypes.txt -Ne@Ne[$i] -i $resultsdir/info.txt -o $resultsdir/out.txt $options");
};
