...
do
	...
	echo >> "${PREPARATION_FILE}" "cp -v ${HAPMAP_DIR}/${DATADIR}/phased_{genotypes,loci}.txt ${DATADIR}"
	echo -n >> "${PREPARATION_FILE}" \
	perl ${CHR22} \
		--mask-data \
		--pop=${POPULATION} \
		--percent-indivs "${PERCENT_INDIVS}" \
		--percent-loci "${PERCENT_LOCI}" \
		--limit-loci "${LOCI_LIMIT}" \
...
do
	...
	do
		...
		echo -n >> "${TASK_FILE}" " ; "
		# echo -n >> "${TASK_FILE}" "cd ${CURRENT_DIR} ; "
		...
		perl ${CHR22} \
			...
			--burnin $(expr "${TRAINING_SAMPLES}" \* ${TRAINING_BURNIN_FACTOR} / "${TRAINING_BURNIN_DIVISOR}" ) \
			--every "${TRAINING_EVERY}" \
			--re-run
		echo -n >> "${TASK_FILE}" " ; "
		# FIXME: Number of individuals
		# predict_time "${TRAINING_SAMPLES}" 100 ${STATES}
		PREDICTED_TIME=$(predict_time 200 100 ${STATES})
		TIME_TOTAL="${TIME_TOTAL} + ${PREDICTED_TIME}"
		...
		perl ${CHR22} \
			...
			--burnin $(expr ${TESTING_SAMPLES} \* "${TESTING_BURNIN_FACTOR}" / "${TESTING_BURNIN_DIVISOR}" ) \
			--mutual-information \
			--every "${TESTING_EVERY}" \
			--re-run
		# new line
