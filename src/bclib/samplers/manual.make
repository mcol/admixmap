###############################################################################################
# makefile for compiling libsampler, the sampling library
#
...
objects		=  rand.o AdaptiveRejection.o StepSizeTuner.o Gaussian.o GaussianProposalMH.o DirichletParamSampler.o HamiltonianMonteCarlo.o MuSampler.o DispersionSampler.o ModeFinder.o
...
shared: paraheader $(objects)
	$(CXX) $(CPPFLAGS) -shared -Wl,-soname,libsampler.so\
-o libsampler.so $(objects) -lgsl -lgslcblas
...
dist:
	...
	mkdir libsampler
	cp *.cc *.h *.txt README libsampler
	cp makefile libsampler
	tar -czf libsampler.tar.gz libsampler
	rm -R libsampler
